...
router.get('/trips', function (req, res, net) {
...
router.get('/card', function (req, res, next) {
  ...
  for (var i = 0; i < travel.length; i++) {
    if (req.query.name === travel[i].name) {
      j = i;
    }
  }
  res.render('card', { travel, name: travel[j].name, image: travel[j].image, description: travel[j].description });
})

router.get('/delete', function (req, res, next) {
  var newTravel = travel.filter(elem => elem.name != req.query.name);
...
router.post('/trips', function (req, res, next) {
  ...
  travel.push({ name: req.body.country, image: req.body.url, description: req.body.description })
  res.render('trips', { travel });
})
